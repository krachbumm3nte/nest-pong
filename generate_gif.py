...
def scale_coordinates(pos):
    ...
    """
    return (int(FIELD_SIZE_SCALED[0]*pos[0]/1.6), int(FIELD_SIZE_SCALED[1]*pos[1]))
...
if __name__ == "__main__":
    ...
    with open(os.path.join(input_folder, "gamestate.pkl"), 'rb') as f:
        game_data = np.array(pickle.load(f))


    game_data[:,:3,0] = game_data[:,:3,0] * FIELD_SIZE_SCALED[0]/1.6
    game_data[:,:3,1] = game_data[:,:3,1] * FIELD_SIZE_SCALED[1]
    game_data = game_data.astype(int)
    game_data[:,:3,0] += FIELD_PADDING
    game_data[:,1,0] -= PADDLE_WID
    ...
    while i < sim_iterations:
        ...
        for (x,y), color in zip(game_data[i][1:3], [left_color, right_color]):
            
            if y < PADDLE_LEN: 
                y = PADDLE_LEN
            elif y >= FIELD_IMAGE_SIZE[1] - PADDLE_LEN:
                y = FIELD_IMAGE_SIZE[1] - PADDLE_LEN
        ...
        if y < PADDLE_LEN: 
            y = PADDLE_LEN
        elif y >= FIELD_SIZE_SCALED[1] - PADDLE_LEN:
            y = FIELD_SIZE_SCALED[1] - PADDLE_LEN
        ...
        if y < PADDLE_LEN: 
            ...
            y = FIELD_SIZE_SCALED[1] - PADDLE_LEN
        playing_field[x:x+PADDLE_WID,y-PADDLE_LEN:y+PADDLE_LEN] = right_color
        """
        ...
        if i % PLOT_INTERVAL == 0 or output_speed > 10:
            # only generate reward plot and heatmaps every PLOT_INTERVAL iterations or every frame when skipping many frames

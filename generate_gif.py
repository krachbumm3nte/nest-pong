...
Notes
------
As the name implies, the output of this script is intended for use in a GIF yet
aggregating images into a GIF using PIL causes some issues with file size and
image quality. Several alternatives exist for this, I can recommend ImageMagick
which can be downloaded `here <https://imagemagick.org/script/download.php>`_ .
To create a gif from the images generated by this script, use (e.g.):

$ convert -delay 15 images/*.png pong_game.gif
...
def grayscale_to_heatmap(in_image, min_val, max_val, base_color):
    """transform a grayscale image to an RGB heat map. Heatmap will color small values in base_color and high values white
...
if __name__ == "__main__":
    ...
    if os.path.exists(out_folder):
        print(f"output folder <{out_folder}> already exists, aborting!")
        sys.exit()
    else:
        os.mkdir(out_folder)
